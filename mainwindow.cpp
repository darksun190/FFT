    ...
    ui(new Ui::MainWindow)
{
    ...
    if (geo_type == QString("Circle") || geo_type == QString("Cylinder"))
    {
    ...
    if (geo_type == QString("Plane") || geo_type == QString("Line"))
    {
        int size = xml_info.names.at(0).points.size();
        Vector <double> zero_point(3);
        zero_point(1) = xml_info.names.at(0).p.x;
        ...
        QVector<point> point_trans; //save the data after alignment transform
        {
            ...
            QPoint start_point(point_trans.first().x,point_trans.first().y);
            QVector<double> length_commu;
            double length_n = 0;
            length_commu.push_back(length_n);

            for (int i=1;i<size;++i)
            {
                length_n +=  hypot(
                            point_trans.at(i).x-point_trans.at(i-1).x,
                            point_trans.at(i).y-point_trans.at(i-1).y
                            );
                length_commu.push_back( length_n );
            }

            {
                //calulating the axial FFT
                Vector< complex <double> >Rk;
                Vector<double>rn;
                rn.resize(size);
                for(int i=0;i<size;++i)
                {
                    rn[i] = point_trans.at(i).z;
                    //rn[i] = hypot(point_trans.at(i).y,point_trans.at(i).x)-radius;
                }
                Rk = fftr2c( rn );
                for (int j=0;j<300;j++)
    ...
    if (geo_type == QString("curve"))
    {
        // copy from the circle, need to modify
        int size = xml_info.names.at(0).mea_points.size();
        QVector<point> point_trans; //save the data after alignment transform
        double radius;       //need calculate, use mid value between the top & bottom
        double lead;         //actual lead for the cylinder
        QVector<double> angle_commu;
        {
            double px,py,pz;    //x y z of the cylinder bottom point
            double cu,cv,cw;    //vector of the cylinder's axis
            {
                //calulate the alignment cylinder for the lift curve
            ...
            {
                point tmp;
                foreach (tmp, xml_info.names.at(0).mea_points)
                {
                    Vector<double> o(3);
                    o(1) = tmp.x;
                    o(2) = tmp.y;
